...
/** An interface for accessing available and selected app icon options, as well as persisting the selected one. */
...
class DefaultAppIconRepository(
    ...
    internal fun getCurrentLauncherAliasSuffix(): String {
        return packageManager.getFenixLauncherName(packageName)?.substringAfter("$packageName.")
...
interface PackageManagerWrapper {
    ...
    /**
     * Retrieves the [ResolveInfo] for the given package if there is one registered, and returns its activity name.
     *
     * @param packageName The package name of the application, e.g. "org.mozilla.fenix"
     */
