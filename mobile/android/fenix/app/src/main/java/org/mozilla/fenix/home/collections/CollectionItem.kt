...
fun CollectionItem(
    ...
    var pendingRemoval by remember(tab.id) { mutableStateOf<Boolean?>(null) }
    ...
    if (pendingRemoval != null) {
        AlertDialog(
            title = { Text(stringResource(string.remove_tab_from_collection)) },
            text = { Text(tab.title.ifBlank { tab.url }) },
            onDismissRequest = { pendingRemoval = null },
            confirmButton = {
                TextButton(
                    onClick = {
                        pendingRemoval?.let { wasSwiped ->
                            pendingRemoval = null
                            onRemove(wasSwiped)
                        }
                    },
                ) {
                    Text(stringResource(string.create_collection_positive))
                }
            },
            dismissButton = {
                TextButton(onClick = { pendingRemoval = null }) {
                    Text(stringResource(string.create_collection_negative))
                }
            },
        )
    }
    ...
    SwipeToDismissBox(
        ...
        Card(
            ...
            FaviconListItem(
                ...
                onIconClick = { pendingRemoval = false },
            )
        }
    }
}
