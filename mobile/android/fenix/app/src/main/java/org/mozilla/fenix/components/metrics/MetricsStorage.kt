...
interface MetricsStorage {
    ...
    /**
     * Will try to register this as a recorder of app usage based on whether usage recording is still needed. It will
     * measure usage by to monitoring lifecycle callbacks from [application]'s activities and should update local state
     * using [updateUsageState].
     */
...
internal class DefaultMetricsStorage(
    ...
    override suspend fun updateSentState(event: Event) =
        withContext(dispatcher) {
            when (event) {
                Event.GrowthData.ConversionEvent1 -> {
    ...
    private fun hasBeenActiveThreeDaysInFirstWeek(): Boolean =
        Result.runCatching {
                val distinctDaysCount =
                    settings.firstWeekDaysOfUseGrowthData
                        .asSequence()
    ...
    internal fun shouldTrackFirstWeekFullActivityDefault(
        ...
        Result.runCatching {
                ...
                val isAllWeekActive =
                    firstWeekDaysOfUse.toTimestamps().count { date ->
    ...
    internal fun activeInFirstPartOfTheWeek(firstWeekDaysOfUse: Set<String>): Boolean =
        firstWeekDaysOfUse.toTimestamps().count { date -> installAge.isDuringFirstFourDays(date) } >=
            MINIMUM_ACTIVE_DAYS_FOR_RECURRENT_ACTIVITY

    @VisibleForTesting
    internal fun activeInLastPartOfTheWeek(firstWeekDaysOfUse: Set<String>): Boolean =
        firstWeekDaysOfUse.toTimestamps().count { date -> installAge.isDuringLastThreeDays(date) } >=
            MINIMUM_ACTIVE_DAYS_FOR_RECURRENT_ACTIVITY
    ...
    internal fun updateIsDefaultBrowserDuringFirstFourDays(
        ...
        val shouldUpdate =
            !isDefaultBrowserDuringFirstFourDay && isDefaultBrowser && installAge.isDuringFirstFourDays(currentTime)
    ...
    private fun Long.toCalendar(): Calendar =
        Calendar.getInstance(Locale.US).also { calendar ->
    ...
    private fun shouldUpdateSearchUsage(): Boolean {
        val currentTime = dateTimeProvider.currentTimeMillis()
        return installAge.isAfterThirdDay(currentTime) && installAge.isDuringFirstWeek(currentTime)
    }

    private fun isInsideGrowthTrackingPeriod(currentTime: Long): Boolean {
        return installAge.isAfterFirstDay(currentTime) && installAge.isDuringFirstMonth(currentTime)
    }
    ...
    companion object {
        ...
        fun getInstalledTime(context: Context): Long =
