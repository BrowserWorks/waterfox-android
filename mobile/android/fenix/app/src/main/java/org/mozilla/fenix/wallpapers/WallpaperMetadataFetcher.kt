...
class WallpaperMetadataFetcher(
    ...
    private fun JSONArray.getAvailableLocales(): List<String>? = (0 until length()).map { getString(it) }
    ...
    /**
     * Normally, if a field is specified in json as null, then optString will return it as "null". If a field is missing
     * completely, optString will return "". This will correctly return null in both those cases so that optional
     * properties are marked as missing.
     */
    private fun JSONObject.optStringOrNull(propName: String) =
        optString(propName).takeIf {
            it != "null" && it.isNotEmpty()
        }

    /**
     * The wallpaper metadata has 6 digit hex color codes for compatibility with iOS. Since Android expects 8 digit ARBG
     * values, we prepend FF for the "fully visible" version of the color listed in the metadata.
     */
    private fun JSONObject.getArgbValueAsLong(propName: String): Long = "FF${getString(propName)}".toHexColor()
